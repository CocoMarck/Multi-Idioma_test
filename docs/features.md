...
## Importante
Durante todo este proceso de crear mi DB para textos pa apps. Me di cuenta, que mi StandarDatabase ta bueno, pero el user `repo x` y `repo y`, en `repo h`, lo puedo evitar un monton, nomas con querys. Y que añadir constantes de name de tables y parameters, ta hasta mejor evitarlo. La proxima ves, intentare solo usar StandardDatabase para crear mis repos de tablas, y usar a full los querys, para evitar depeder de repos en un repo. Es lo mas legible, y lo mejor para el coco del pc.
